            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="relative">
                <div className="absolute left-4 top-1/2 -translate-y-1/2 flex items-center gap-2 text-gray-400 pointer-events-none">
                  ...
                  {!form.date && <span>Select Date</span>}
                </div>
                <input
                  type="date"
                  value={form.date}
                  onChange={(e) => setField("date", e.target.value)}
                  className={`${inputStyle} ${!form.date ? "text-transparent" : "text-gray-700"}`}
                  style={{ paddingLeft: form.date ? "2.8rem" : "1rem" }}
                />
              </div>
              <div className="relative">
                <div className="absolute left-4 top-1/2 -translate-y-1/2 flex items-center gap-2 text-gray-400 pointer-events-none">
                  ...
                  {!form.time && <span>Select Time</span>}
                </div>
                <input
                  type="time"
                  value={form.time}
                  onChange={(e) => setField("time", e.target.value)}
                  className={`${inputStyle} ${!form.time ? "text-transparent" : "text-gray-700"}`}
                  style={{ paddingLeft: form.time ? "2.8rem" : "1rem" }}
                />
              </div>
            </div>
          </div>
